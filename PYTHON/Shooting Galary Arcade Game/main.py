...
pg.display.set_caption('Shooting Gallery')

# load images
images = {}
def load_image(name, filename, flip_x = False):
    images[name] = pg.image.load(filename).convert_alpha()

    # flip image on the x-axis
    if flip_x:
        images[name] = pg.transform.flip(images[name], True, False)
...
load_image('score', 'images/text/text_score_small.png')
load_image('colon', 'images/text/text_dots_small.png')
load_image('gameover', 'images/text/text_gameover.png')

# load the number images
for i in range(10):
    load_image(str(i), f'images/text/text_{i}_small.png')
...
# sprite groups
brown_duck_group = pg.sprite.Group()
yellow_duck_group = pg.sprite.Group()
...
while running:
    ...
    # draw the crosshair
    crosshair_x, crosshair_y = pg.mouse.get_pos()
